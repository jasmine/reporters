...
class ConsoleReporter {
  ...
  jasmineDone(result) {
    ...
    if (this.#notApplicableSpecs.length > 0) {
      const byReason = {};

      for (const spec of this.#notApplicableSpecs) {
        if (byReason[spec.notApplicableReason]) {
          byReason[spec.notApplicableReason]++;
        } else {
          byReason[spec.notApplicableReason] = 1;
        }
      }
      ...
      for (const [reason, count] of Object.entries(byReason)) {
        if (count === 1) {
          ...
          this.#print(`${count} specs`);
        }

        this.#print(` not applicable because: ${reason}`);
        this.#printNewline();
      }
    }
  ...
  specDone(result) {
    ...
    switch (result.status) {
      ...
      case 'notApplicable':
        this.#notApplicableSpecs.push(result);
        this._executableSpecCount++;
        break;
