...
describe('ConsoleReporter', function() {
  ...
  it('allows the seed reproduction command to be specified', function() {
    ...
    expect(this.out.getOutput()).toMatch(
      /Randomized with seed 12345 \(jasmine-some-other-tool --randomSeed=12345\)/
    );
  });
  ...
  it('reports a summary without a "no expectations" message for a spec having passed expectations', function() {
    ...
    reporter.specDone({
      ...
      description: 'with a failing spec',
      fullName:
        'A suite with a failing spec that has both passed and failing expectations',
      failedExpectations: [],
      passedExpectations: [
        {
          passed: true,
  ...
  it('displays all afterAll exceptions', function() {
    ...
    expect(this.out.getOutput()).toMatch(
      /Suite error: suite 1\s+Message:\s+After All Exception/
    );
    expect(this.out.getOutput()).toMatch(
      /Suite error: suite 2\s+Message:\s+Some Other Exception/
    );
    expect(this.out.getOutput()).toMatch(
